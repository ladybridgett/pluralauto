(function (plugin, vendetta) {
  ...
  function expectProxyMessage(channelId, applicationId) {
    if (!channelId) return;
    pendingProxyMessages[String(channelId)] = {
  ...
  function isExpectedProxyMessage(channelId, message) {
    var key = String(channelId || "");
    var pending = pendingProxyMessages[key];
    var ids;
    var matches = false;
    var hasInteraction;
    if (!pending) return false;
    if (Date.now() > pending.expiresAt) {
      delete pendingProxyMessages[key];
      return false;
    }
    ...
    pending.remaining -= 1;
    if (pending.remaining <= 0) delete pendingProxyMessages[key];
    return true;
  }
  ...
  function localProxyMessage(message, channelId) {
    ...
    delete local.interaction_data;
    delete local.interactionData;
    delete local.interaction_context;
  ...
  function attachLocalProxyHandling() {
    ...
    try {
      ...
      receiveMessageUnpatch = function () {
        var cleanup;
        while (unpatches.length) {
          cleanup = unpatches.pop();
  ...
  function findSendButtonPress(rendered) {
    var React = metro.common.React;
    var found = null;
    ...
    function visit(node) {
      ...
      if (typeof props.onPress === "function") {
        found = {
          ...
          accessibilityLabel: props.accessibilityLabel || "Send message"
        };
        return;
      }
  ...
  function afterSpinnerPaint(callback) {
    if (typeof requestAnimationFrame === "function") {
      requestAnimationFrame(function () {
  ...
  function ProxySendButton(props) {
    ...
    var loadingState = React.useState(isChannelSending(channelId));
    var loading = loadingState[0];
    var setLoading = loadingState[1];
    var pressInfo = findSendButtonPress(props.rendered);
    var indicator;

    React.useEffect(function () {
      setLoading(isChannelSending(channelId));
      return subscribeToChannelSending(channelId, function () {
        setLoading(isChannelSending(channelId));
      });
    }, [channelId]);

    if (!loading && !isChannelSending(channelId)) {
      if (!pressInfo || !RN.Pressable) return props.rendered;
      return React.createElement(
        RN.Pressable,
        {
          accessibilityRole: "button",
          accessibilityLabel: pressInfo.accessibilityLabel,
          disabled: pressInfo.disabled,
          android_ripple: null,
          onPress: function (event) {
            var pressToken = primePressedSending(channelId);
            if (event && typeof event.persist === "function") {
              event.persist();
            }
            setLoading(true);
            afterSpinnerPaint(function () {
              try {
                pressInfo.onPress(event);
              } catch (error) {
  ...
  function isEmojiOnlyText(value) {
    ...
    compact = compact
      ...
      .replace(/[\u200D\uFE0E\uFE0F]/g, "");
    return Boolean(original) && compact === "";
  }

  function enlargeEmojiOnlyReply(content, replyTarget) {
    var trimmed;
    if (!replyTarget || !isEmojiOnlyText(content)) return content;
    trimmed = String(content || "").replace(/^\s+|\s+$/g, "");
    return "# " + trimmed;
  }
  ...
  function intercept(args, original) {
    ...
    try {
      ...
      content = enlargeEmojiOnlyReply(content, replyTarget);
      ...
      operation = findCommand(
        ...
        .then(function () {
          expectProxyMessage(
            channelId,
            commandApplicationId(proxyCommand) ||
              selectedProxy.applicationId
          );
