(function (plugin, vendetta) {
  ...
  function executeCommand(command, content, channelId, optionName) {
    var channel = getChannel(channelId);
  ...
  plugin.settings = function Settings() {
    ...
    children.push(
      React.createElement(
        ...
        "Automatic /plu/ral userproxy commands for DMs."
      )
    );
    ...
    children.push(
      React.createElement(
        ...
        "Attachments and stickers are sent normally. A proxy error blocks the message unless the unsafe fallback is enabled."
      )
    );
